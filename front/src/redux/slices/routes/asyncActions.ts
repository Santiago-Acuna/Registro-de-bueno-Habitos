 ...
 */
const getComponentName = (habitType: 'complex' | 'simple' | 'withoutintervals'): string => {
...
const transformResponse = (data: HabitsByTypeResponse): RouteConfig[] => {
  ...
  const habitTypes: Array<keyof HabitsByTypeResponse> = ['complex', 'simple', 'withoutintervals'];
 ...
 */
export const fetchRoutes = createAsyncThunk<RouteConfig[], void, { rejectValue: string }>(
  'routes/fetchRoutes',
